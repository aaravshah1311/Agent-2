"""Back-compat shim: this module moved to `agent2.server.sockets`.
...
`agent2.sockets.X` and `agent2.server.sockets.X` are the SAME object.
"""

from agent2.server.sockets import *          # noqa: F401,F403

# Underscored names `import *` skips, kept for callers that
# reference them directly (tests, sibling modules).
from agent2.server.sockets import (  # noqa: F401
    _dispatch_agent,
)
